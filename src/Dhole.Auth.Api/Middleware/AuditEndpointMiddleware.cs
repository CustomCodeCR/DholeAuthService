...
namespace Dhole.Auth.Api.Middleware;
...
)
{
    ...
    [
        "id", "entityId", "globalEntityId", "userId", "roleId", "scopeId", "sessionId", "fileId",
        "providerId", "catalogGroupId", "catalogItemId", "departmentId", "departmentTypeId", "menuId",
        "menuItemId", "routeId", "parameterId", "featureFlagId",
    ];
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            logger.LogError(exception, "Failed to audit Auth action {Method} {Path}.", context.Request.Method, context.Request.Path.Value);
        }
    }
    ...
    private static string ResolveEventType(HttpContext context)
        => $"auth.http.{ResolveEntityType(context)?.ToLowerInvariant() ?? "request"}.{ResolveAction(context)}";
